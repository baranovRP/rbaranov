...
public class CatalogDaoImpl implements CatalogDao {
    ...
    public Long create(final CatalogDto catalog) {
        ...
        try (PreparedStatement pstmt =
                 conn.prepareStatement(RB_SQL.getString("insert.catalog"))) {
    ...
    public boolean update(final CatalogDto catalog) {
        ...
        try (PreparedStatement pstmt =
                 conn.prepareStatement(RB_SQL.getString("update.catalog.by.id"))) {
            pstmt.setLong(1, catalog.getUserId());
            pstmt.setLong(2, catalog.getGenreId());
    ...
    public boolean deleteById(final Long id) {
        boolean result = false;
        try (PreparedStatement pstmt =
                 conn.prepareStatement(RB_SQL.getString("delete.catalog.by.id"))) {
            pstmt.setLong(1, id);
    ...
    public boolean deleteAllByUserId(final Long id) {
        boolean result = false;
        try (PreparedStatement pstmt =
                 conn.prepareStatement(RB_SQL.getString("delete.catalogs.by.user"))) {
            pstmt.setLong(1, id);
            int rowCounter = pstmt.executeUpdate();
            conn.commit();
            if (rowCounter == 1) {
